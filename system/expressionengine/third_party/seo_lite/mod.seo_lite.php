...
class Seo_lite {
	...
	{		
        ...
        $url_title = $this->get_param('url_title');
				$category_title = $this->get_param('category_title');
        ...
        if($entry_id || $url_title)
        {
            ...
            if($q->num_rows() > 0)
            {
				...
				if($category_title){
					...
					$this->EE->db->select('cat_name as category_name, cat_description as description, default_keywords, default_description, default_title_postfix, template');
					$this->EE->db->from('categories');
					$this->EE->db->from('seolite_config');
					$where['cat_url_title'] = $category_title;
					$where['group_id'] = 2;
					$this->EE->db->where($where);
					$q = $this->EE->db->get();
					...
					if($q->num_rows() > 0){
						$seolite_entry = $q->row();
						$vars = array(
							'title' => $seolite_entry->category_name,
							'meta_keywords' => $this->get_preferred_value($seolite_entry->default_keywords, $seolite_entry->default_keywords, $default_keywords) ,
							'meta_description' => $this->get_preferred_value(substr($seolite_entry->description,0,200), $seolite_entry->default_description, $default_description),
						);

...
class Minimee {
	...
	private function _abort($e = FALSE)
	{
		...
		// should we add our debug message to html output?
		if($this->debug == 'yes')
		{
			$out = '<!-- ' . $log . " -->\n";
		}
	...
	private function _cache($filename, $file_data)
	{
		...
		log_message('debug', 'Minimee: Cache file ' . $filename . ' was written to ' . $this->cache_path);
	}
	// END
	...
	private function _flightcheck()
	{
		...
		switch ('flightcheck') :
			...
			default :
				log_message('debug', 'Minimee has passed flightcheck.');
			break;
	...
	private function _set_remote_mode()
	{
		...
		{
			log_message('debug', 'Minimee is using CURL for remote files.');
		...
		{
			log_message('debug', 'Minimee is using file_get_contents() for remote files.');
			$this->remote_mode = 'fgc';

			if ( ! defined('OPENSSL_VERSION_NUMBER'))
			{
				log_message('debug', 'Minimee has noticed that your PHP compile does not appear to support file_get_contents() over SSL.');
			}
	...
	private function _tag($filename, $cache = TRUE)
	{
		...
		$url = ($cache) ? $this->EE->functions->remove_double_slashes($this->cache_url . '/' . $filename) : $filename;
